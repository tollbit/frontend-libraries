...
**Description**: Gets all tiles available to the given agent

**Parameters**: N/A

**Returns**: `Promise<TileResponse[]>`
...
**Description**: Given a cuid for a tile, returns the fully resolved tile

**Parameters**:

| Name         | Type     | Description                                                   |
| ------------ | -------- | ------------------------------------------------------------- |
| `cuid`       | `string` | The cuid that corresponds to the tile that is being requested |
| `parameters` | `string` | A valid token generated by [generateToken()](#generatetoken)  |

**Returns**: `Promise<TileResponse | null>`
...
const tiles = await client.getTile("cuid", {
  location: "Boston, MA",
  timePeriod: "3 days",
});
```
